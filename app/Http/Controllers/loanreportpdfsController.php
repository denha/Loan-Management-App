 ...
 class loanreportpdfsController extends Controller{
    ...
    public function allloanspdfs($date1,$date2){
        ...
        if($date1=='day' && $date2=='day'){
            $loans=DB::select("select loanofficer,areamanager,compliance, loanid,DATE_FORMAT(date,'%d-%m-%Y') as date,acno,memid,paydet,customers.name,concat(loaninterest,' ','%' )as loaninterest,concat(loanrepay,' ','month (s)') as loanrepay,collateral,FORMAT(loancredit,0) as loancredit,guanter,headerid from customers inner join loaninfos on loaninfos.memeberid=customers.id inner join loantrans on loantrans.loanid=loaninfos.id inner join loanapproved on loanapproved.memeberid=loaninfos.memeberid where loantrans.branchno=$branch and  isLoan=1 and isDisbursement=1 and loantrans.isActive=1 AND date=$today");
            ...
            $pdf = \App::make('dompdf.wrapper');
            $pdf->loadHTML(view('allloanspdf/index')->with('company',$company)->with('loans',$loans)->with('total',$total))->setPaper('a4', 'landscape');
            return $pdf->stream();
            ...
            $loans=DB::select("select loanofficer,areamanager,compliance, loanid,acno,DATE_FORMAT(date,'%d-%m-%Y') as date,memid,paydet,customers.name,concat(loaninterest,' ','%' )as loaninterest,concat(loanrepay,' ','month (s)') as loanrepay,collateral,FORMAT(loancredit,0) as loancredit,guanter,headerid from customers inner join loaninfos on loaninfos.memeberid=customers.id inner join loantrans on loantrans.loanid=loaninfos.id inner join loanapproved on loanapproved.memeberid=loaninfos.memeberid  where loantrans.branchno=$branch and  isLoan=1 and isDisbursement=1 and loantrans.isActive=1 AND date Between '$date1' AND '$date2' ");
            ...
            $pdf = \App::make('dompdf.wrapper');
            $pdf->loadHTML(view('allloanspdf/index')->with('loans',$loans)->with('total',$total)->with('company',$company))->setPaper('a4', 'landscape');
            return $pdf->stream();
        }
