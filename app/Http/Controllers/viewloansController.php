...
use Illuminate\Http\Request;
 ...
 class viewloansController extends Controller{
   ...
   // if($admin==0){
    ...
    $sth =  DB::getPdo()->prepare("select loanofficer,areamanager,compliance, loanid,DATE_FORMAT(date,'%d-%m-%Y') as date,memid,paydet,customers.name,concat(loaninterest,' ','%' )as loaninterest,concat(loanrepay,' ','month (s)') as loanrepay,collateral,FORMAT(loancredit,0) as loancredit,guanter,headerid from customers inner join loaninfos on loaninfos.memeberid=customers.id inner join loantrans on loantrans.loanid=loaninfos.id inner join loanapproved on loanapproved.memeberid=loaninfos.memeberid where loantrans.branchno=$bra and  isLoan=1 and isDisbursement=1 and loantrans.isActive=1 $where limit $offset,$rows");
    $sth->execute();
