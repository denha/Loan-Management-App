...
public function save(Request $request){ 
    ...
    $mem=DB::select("select loan,isFunded,intmethod,loancat,customers.name as client,sum(loan) as loanbal,sum(interestcredit) as creditinterest,abs(sum(if(interestcredit<0,interestcredit,0))) as interestcredit,abs(sum(if(loan<0,loan,0))) as loanpaid,abs(sum(if(loan<0,loan,0))) as loan,loanid from loantrans inner join customers on loantrans.memid=customers.id inner join loaninfos on loantrans.loanid=loaninfos.id where loantrans.isActive=1 and loantrans.branchno=$branch and memid=$id group by loanid order by loanid desc limit 1");
...
if(str_replace( ',', '',$request['amount'])>0){
                   ...
                   $funded = false;
                    if($memberid->isFunded==1){
                        $funded = true; 
                    }
                  
                   $loans_id = $memberid->loanid;
                   $intial_savings=0;
                   $savings_payments=0;

                   $loan_results = DB::select("select sum(intialSavings) as intialSavings,sum(savingpayment) as savingpayment from topupsavings where loanid = $loans_id");
                    foreach($loan_results as $results){
                        $intial_savings = $results->intialSavings;
                        $savings_payments = $results->savingpayment;
                    }
                    $isCalculate=false;
                    if($intial_savings != $savings_payments){
                        $isCalculate = true;
                    }

                   if($funded){
                        //$funded_savings = $loan*0.2;
                        $funded_loan = ($loan-($loan*0.2));
                        if($isCalculate){
                            if($savings_payments+$funded_loan<=$intial_savings){
                                //$isCalculate =true;
                                $funded_savings = $loan*0.2;
                                $funded_loan = $loan; //afterproblem ($loan-($loan*0.2));
                            }else{
                                $new_saving = $intial_savings-$savings_payments;
                                $balance = ($loan*0.2) - $new_saving;
                                $funded_savings = $new_saving;
                                $funded_loan = $loan; //afterproblem$loan-$new_saving;
                            }
                        }else{
                            $funded_loan = $loan;
                            $funded_savings = 0;
                   ...
                   if($loanpdts->loanpdt=="loanpdt1"){
                    ...
                    $objallsavings->loanint1=$interest*-1;
                    $objallsavings->loanpdt1=$funded_loan*-1;
                    $objallsavings->headerid=$objpurchaseheaders->id;
                    $objallsavings->savingpdt1 = $funded_savings;
                    //$objallsavings->save();
                 }
                 if($loanpdts->loanpdt=="loanpdt2"){
                    $objallsavings->narration="Interest & Loan Repayment";
                 ...
                 if($loanpdts->loanpdt=="loanpdt5"){
                   ...
                   // $objallsavings->save();
                              ...
                              // $objloantrans->save();
                                ...
                                //$objloaninfo->save();
                            // Denis
                            if($funded && $isCalculate){
                                $objtopupsavings = new topupsavings();
                                $objtopupsavings->headerid= $objpurchaseheaders->id;
                                $objtopupsavings->loanid = $memberid->loanid;
                                $objtopupsavings->savingpayment=$funded_savings;
                                //$objtopupsavings->intialSavings = str_replace( ',', '',$request['savings']);
                                $objtopupsavings->save();
                                // compulsary savings 
                                $objaccountrans7=new accounttrans;
                                $objaccountrans7->purchaseheaderid=$objpurchaseheaders->id;
                                $objaccountrans7->amount=$funded_savings;
                                $objaccountrans7->total=$funded_savings;
                                $objaccountrans7->accountcode="400";//$loanpdts->disbursingac;// Disburing Account
                                $objaccountrans7->narration=$memberid->client." -savings";
                                $objaccountrans7->ttype="C";
                                $objaccountrans7->transdate=date("Y-m-d", strtotime($request['date']));
                                $objaccountrans7->bracid=auth()->user()->branchid;
                               // $objaccountrans->save();
                                // Inserting into income account
                                $objaccountrans8=new accounttrans;
                                $objaccountrans8->purchaseheaderid=$objpurchaseheaders->id;
                                $objaccountrans8->amount=$funded_savings;
                                $objaccountrans8->accountcode=$loanpdts->disbursingac;
                                $objaccountrans8->ttype="D";
                                $objaccountrans8->total=$funded_savings;
                                $objaccountrans8->narration=$memberid->client." -savngs";
                                $objaccountrans8->transdate=date("Y-m-d", strtotime($request['date']));
                                $objaccountrans8->bracid=auth()->user()->branchid;
                                ...
                                $objaccountrans1=new accounttrans;
                                $objaccountrans1->purchaseheaderid=$objpurchaseheaders->id;
                                $objaccountrans1->amount=$interest+$funded_loan;
                                $objaccountrans1->total=$interest+$funded_loan;
                               ...
                               // $objaccountrans->save();
                                ...
                                $objaccountrans3=new accounttrans;
                                $objaccountrans3->purchaseheaderid=$objpurchaseheaders->id;
                                $objaccountrans3->amount=abs($funded_loan);
                                $objaccountrans3->total=$funded_loan*-1;
...
$Objgroup = new indpayments();
$Objgroup->date=date("Y-m-d", strtotime($request['date']));
$Objgroup->grpid=$group_id;
$Objgroup->head=$objpurchaseheaders->id;
$file=$request->file('proof');
...
DB::commit();
 ...
 public function destroy(Request $request, $headerid,$delloan,$isdelsec,$idloan){
   ...
   // Deleting from interest
    ...
    DB::delete("delete from topupsavings where headerid=$headerid");
    ...
    public function destroyindu(Request $request,$head){
      ...
      // Deleting from interest
       ...
       DB::delete("delete from topupsavings where headerid='$head'");
        }catch(\Exception $e){
            echo "Failed ".$e;
            DB::rollback();
