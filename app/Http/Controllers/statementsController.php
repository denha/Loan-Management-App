...
public function viewstockreport($where){
   ...
   $results=array();
    ...
    FROM savings AS B inner join customers on customers.id=B.client_no $where having abs(moneyin+moneyout)>0   ORDER BY B.id asc ");
...
public function allsavings1($where){
    ...
    $results['total']=$krows[0]->count;
    $rst =  DB::getPdo()->prepare("select format(surcharge,0) as surcharge,id, DATE_FORMAT(date,'%d-%m-%Y') date,narration,recieptno,format(savingpdt1,0)as savingpdt1,format(intpdt1,0)as intpdt1,format(savingpdt2,0)as savingpdt2,format(intpdt2,0)as intpdt2,format(savingpdt3,0)as savingpdt3,format(intpdt3,0)as intpdt3,format(savingpdt4,0)as savingpdt4,format(intpdt4,0)as intpdt4,format(savingpdt5,0)as savingpdt5,format(intpdt5,0)as intpdt5,
    format(loanpdt1,0)as loanpdt1,format(loanint1,0)as loanint1,format(loanpdt2,0)as loanpdt2,format(loanint2,0)as loanint2,format(loanpdt3,0)as loanpdt3,format(loanint3,0)as loanint3,format(loanpdt4,0)as loanpdt4,format(loanint4,0)as loanint4,format(loanpdt5,0)as loanpdt5,format(loanint5,0)as loanint5,format(shares,0)as shares,format(ansub,0) as ansub, format(memship,0) as memship,headerid as head  from allsavings $where limit  $offset,$rows");
    $rst->execute();

    $viewall = $rst->fetchAll(\PDO::FETCH_OBJ);
   $results['rows']=$viewall;
   //Showing The footer and totals 
 $footer =  DB::getPdo()->prepare("select format(sum(surcharge),0) as surcharge, format(sum(savingpdt1),0)as savingpdt1,format(sum(intpdt1),0)as intpdt1,format(sum(savingpdt2),0)as savingpdt2,format(sum(intpdt2),0)as intpdt2,format(sum(savingpdt3),0)as savingpdt3,format(sum(intpdt3),0)as intpdt3,format(sum(savingpdt4),0)as savingpdt4,format(sum(intpdt4),0)as intpdt4,format(sum(savingpdt5),0)as savingpdt5,format(sum(intpdt5),0)as intpdt5,
