...
public function trialbalancepdfs($date1,$date2,$day1,$day2){
    ...
    $company=DB::select("select * from companys where id=$branch");
    $pdf = \App::make('dompdf.wrapper');
    DB::statement("CREATE OR REPLACE VIEW  trialfooter as  SELECT transdate,accountname,accountcode, if(Sum(If(ttype='D',amount,-amount))<0,0,Sum(If(ttype='D',amount,-amount))) as Debits,if(Sum(If(ttype='D',amount,-amount))>0,0,abs(Sum(If(ttype='D',amount,-amount)))) as Credits  from `trialpdfs` where bracid=$branch and  transdate between '$date1' AND '$date2' $thedate group by accountcode");
    $trial=DB::select("SELECT transdate,accountname,accountcode, format(if(Sum(If(ttype='D',amount,-amount))<0,0,Sum(If(ttype='D',amount,-amount))),0) as Debits,format(if(Sum(If(ttype='D',amount,-amount))>0,0,abs(Sum(If(ttype='D',amount,-amount)))),0) as Credits  from `trialpdfs` where bracid=$branch and  transdate between '$date1' AND '$date2' $thedate group by accountcode");
