...
public function balShit($where){
   ...
   //DB::statement("drop view Bsheet");
     DB::statement("create or replace  view Bsheet as select accountcode,bracid,ttype,If(ttype='D',amount,-amount)as Debit,transdates from accounttrans inner join purchaseheaders on purchaseheaders.id=accounttrans.purchaseheaderid where transdates $where and bracid=$branch");
