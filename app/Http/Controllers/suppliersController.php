...
public function save(Request $request){
    ...
    try{
        ...
        $grpCheckId=0;
        ################# checking which group does a memeber belong to ###############
        $groupresults=groupmembers::where('memberid','=',$memberid)->get();
        foreach($groupresults as $results){
            $grpCheckId=$results->groupid;
        }
        $checked=DB::select("select groupid,memberid,name from groupmembers inner join loaninfos on loaninfos.memeberid=groupmembers.memberid inner join customers on customers.id=groupmembers.memberid where isApprove=3 and groupid=$grpCheckId");
        if(count($checked)>0){
            foreach($checked as $check){
        return ['reject'=>'yes','names'=>$check->name];
            }

        }else{
        $clientname="";
        $groupid=0;
        $id=DB::select("select isFunded,customers.name as client,sum(loan) as loan from loantrans inner join customers on loantrans.memid=customers.id where loantrans.isActive=1 and memid=$memberid");
...
foreach($groupdata as $data){
    $groupid=$data->id;
        ...
        // inserting into cycles
        $objcycles = new cycles();
        $objcycles->memid=$request['name'];
        $objcycles->header=$objheaders->id;
        $objcycles->cycle=1;
        $objcycles->save();
        ...
        // getting loanofficers who instialled 
        $appr=loaninfo::find($request['appid']);
       // $objloaninfo->isApprove=2;
       $objloaninfo->loanofficer=$appr->loanofficer;
        ...
        $non_funded =true;
        if($finalid->isFunded ==1){
            $non_funded= false; 
        }
        if($request['method']==2){
            // if first time or non funded 
            ...
            if($non_funded){
                ...
                $savings=str_replace( ',', '',$request['savings']);
                $objtopupsavings = new topupsavings();
                $objtopupsavings->headerid= $objheaders->id;
                $objtopupsavings->loanid = $objloaninfo->id;
 ...
 $objallsavingsInsurance->narration="Insurance & Compulsory Savings ";
     ...
     if($objloancat->loanpdt=="loanpdt1"){
       ...
       if($non_funded){
        ...
        $objallsavings->savingpdt1=str_replace( ',', '',$request['savings']);

                 //inserting into cash Account 
        $objaccountrans=new accounttrans;
        $objaccountrans->purchaseheaderid=$objheaders->id;
        $objaccountrans->amount=str_replace( ',', '',$request['savings']);
        $objaccountrans->accountcode=21;
        $objaccountrans->narration= "Cash Deposit";
        $objaccountrans->ttype="D";
        $objaccountrans->total=str_replace( ',', '',$request['savings']);
        $objaccountrans->transdate=date("Y-m-d", strtotime($request['date']));
        $objaccountrans->bracid=$branchid;
        $objaccountrans->save();
        ...
        $objaccountrans->purchaseheaderid=$objheaders->id;
        $objaccountrans->amount=str_replace( ',', '',$request['savings']);
        $objaccountrans->accountcode=400;
        $objaccountrans->narration= "Cash Deposit";
        $objaccountrans->ttype="C";
        $objaccountrans->total=str_replace( ',', '',$request['savings']);
        $objaccountrans->transdate=date("Y-m-d", strtotime($request['date']));
...
$user=auth()->user()->name;
  ...
  // DB::delete("delete from loaninfos where id='$id'");
   ...
   DB::delete("delete from cycles where header=$headerid");
