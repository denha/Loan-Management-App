...
public function viewjournelreport(){
    if(isset($_GET['page'])&& isset($_GET['rows']) && empty($_GET['date1']) && empty($_GET['date2']) && empty($_GET['branch'])){
        $branch=$branch= auth()->user()->branchid;;//$_GET['branch'];
        $today=date("'Y-m-d'");
$this->view(" and transdates=$today and chartofaccounts.branchno=$branch order by purchaseheaderid asc");
    }
    if(isset($_GET['page'])&& isset($_GET['rows'])  && !empty($_GET['date1']) && !empty($_GET['date2']) && !empty($_GET['branch'])){
        ...
        $branch=$branch= auth()->user()->branchid;;//$_GET['branch'];
        $this->view("and transdates  BETWEEN '$date1' AND '$date2' and chartofaccounts.branchno=$branch order by purchaseheaderid asc");
