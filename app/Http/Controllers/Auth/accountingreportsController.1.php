...
use Illuminate\Support\Facades\DB;
 ...
 class accountingreportsController extends Controller{
  ...
  /* if(isset($_GET['page'])&& isset($_GET['rows']) && !empty($_GET['start']) && !empty($_GET['end']) && !empty($_GET['date1']) && !empty($_GET['date2']) &&isset($_GET['account'])){   
    ...
    $this->report("where purchaseheaders.isActive=1 and transdates BETWEEN '$date1' AND '$date2' AND transdates BETWEEN '$start' AND '$endFin' AND accounttrans.accountcode=$account","sum(if(ttype='D',amount,''))as amount,sum(if(ttype='D',amount,'')) as debits,sum(if(ttype='C',amount,'')) as credits ");
  ...
  /* if(isset($_GET['page'])&& isset($_GET['rows']) && !empty($_GET['start']) && !empty($_GET['end']) && !isset($_GET['date1']) && !isset($_GET['date2']) && isset($_GET['account']) && !isset($_GET['branch']))  {
    $this->report("where purchaseheaders.isActive=1 and transdates = $today AND transdates BETWEEN '$start' AND '$endFin' AND accounttrans.accountcode=$account ","sum(if(ttype='D',amount,''))as amount,sum(if(ttype='D',amount,'')) as debits,sum(if(ttype='C',amount,'')) as credits ");
...
public function viewincomestats(){
 ...
 public function trialbalance(){
   ...
   $this->viewtrial("where transdates BETWEEN '$start' AND $today AND transdates BETWEEN '$start' AND '$endFin' and purchaseheaders.isActive=1 ");
        $this->Strialbalance();
 ...
 public function trialandsheet($sql,$count,$footer){
   ...
   echo json_encode($results);
    ...
    public function report($where,$total){
        ...
        $offset = ($page-1)*$rows;
        $rs =DB::getPdo();
        $krows = DB::select("select COUNT(*) as count,code,name, purchaseheaders.id ,DATE_FORMAT(transdates,'%d-%m-%Y') as transdates,chartofaccounts.accountname,B.accountcode,narration,format(if(ttype='D',amount,''),0)as debits,format(if(ttype='C',amount,''),0)as credits,format(amount,0) as amount,(SELECT format(SUM(A.total),2) FROM accounttrans AS A where  A.id <= B.id ORDER BY B.id  asc )as runningbal from accounttrans as B INNER join purchaseheaders on B.purchaseheaderid=purchaseheaders.id inner join chartofaccounts on chartofaccounts.accountcode=B.accountcode inner join accounttypes on accounttypes.id=chartofaccounts.accounttype inner join editexpenses on B.purchaseheaderid=editexpenses.id $where");
        $results['total']=$krows[0]->count;
