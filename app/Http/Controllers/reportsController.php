...
public function viewindeport($where){
    ...
    $results=array();
     ...
     $offset = ($page-1)*$rows;
     $rs =DB::getPdo();
     $krows = DB::select("select COUNT(*) as count from loantrans inner join customers on loantrans.memid=customers.id and loantrans.isActive=1  where memid= $where order by loanid");
     $results['total']=$krows[0]->count;
     //$rst =  DB::getPdo()->prepare("select DATE_FORMAT(date,'%d-%m-%Y') as date,if(expecteddate='0000-00-00','',DATE_FORMAT(expecteddate,'%d-%m-%Y')) as expecteddate,format(loan,0)as loan,format(interestcredit,0) as interestcredit,narration, name, format(loancredit,0) as loancredit from loantrans inner join customers on loantrans.memid=customers.id and loantrans.isActive=1  $where order by date  limit  $offset,$rows");
     $rst =  DB::getPdo()->prepare("select loanid, DATE_FORMAT(date,'%d-%m-%Y') as date,if(expecteddate='0000-00-00','',DATE_FORMAT(expecteddate,'%d-%m-%Y')) as expecteddate,format(loan,0)as loan,format(interestcredit,0) as interestcredit,narration,format(abs(loan+interestcredit),0) as total, name, format(loancredit,0) as loancredit,(SELECT format(SUM(A.loancredit),2) FROM loantrans AS A where memid= $where AND  A.id <= B.id ORDER BY B.id  asc )as runningbal  from loantrans As B inner join customers on B.memid=customers.id where memid= $where order by B.id asc  limit  $offset,$rows");
     $rst->execute();
 
     $viewall = $rst->fetchAll(\PDO::FETCH_OBJ);
    $results['rows']=$viewall;
    //Showing The footer and totals 
    $footer =  DB::getPdo()->prepare("select format(sum(loancredit),0) as loancredit, format(sum(loan),0) as loan ,format(sum(interestcredit),0) as interestcredit  from loantrans inner join customers on loantrans.memid=customers.id and loantrans.isActive=1  where memid= $where order by loanid   limit  $offset,$rows");
    $footer->execute();
    $foots=$footer->fetchAll(\PDO::FETCH_OBJ);
    $results['footer']=$foots;
    echo json_encode($results);
