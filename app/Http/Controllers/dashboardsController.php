...
public function index(){
   ...
   DB::beginTransaction();
   try{
       // invoking surchage computation
       $this->defaultedgrps(); // Defaulter groups
       $this->currentPayment();
    $this->notesfixed();
...
public function three(){
    $bra=auth()->user()->branchid;
    $months= DB::select("select name,acno,format(savingpdt1,0)as savingpdt1,client_no,max(DATE_FORMAT(date,'%d-%m-%Y')) as date from allsavings inner join customers on customers.id=allsavings.client_no where savingpdt1>0 and customers.branchnumber=$bra  group by client_no order by max(date)");
    $a=0;
...
public function defaultedgrps(){
    $results=array();
    $bra=auth()->user()->branchid;
    $data=DB::select("select groups.name, sum(surcharge) as total from allsavings inner join groupmembers on groupmembers.memberid=allsavings.client_no inner join groups on groups.id=groupmembers.groupid where branchid=$bra group by groups.id  having total>1 order by name asc");
   $fixes= DB::select("select count(id) as defaulted from (select allsavings.id as id, sum(surcharge) as total from allsavings inner join groupmembers on groupmembers.memberid=allsavings.client_no inner join groups on groups.id=groupmembers.groupid where branchid=$bra group by groups.id having total>1) t");
   $today=date('Y-m-d');
   foreach($fixes as $fix){
...
/*
   ...
   foreach($groups as $groupz){
       $gid=$groupz->groupid;
       $grpmem=DB::select("select sum(penalty) as penz,groupid,count(groupid) as no ,scheduledate,expecteddate  from (select penalty,groupid,scheduledate,expecteddate from pen where groupid=$gid order by loanid desc limit 5 ) t");
       ...
       foreach($grpmem as $mems){
           ...
           //geting payment mad
           if($mems->no==5){
           $payments=DB::select("select groupid,sum(loancredit) as loancredit,memid from loantrans inner join groupmembers on groupmembers.memberid=loantrans.memid where  groupid=$newgid");
           foreach($payments as $pay){
               //echo $pay->loancredit;
               ##### TODAY #####
               $paidAmount=($mems->penz)*16-($pay->loancredit);
            
               if($paidAmount<($mems->penz)*16){ //checking is loans are complete if not complete process
...
//echo $news;
//echo "The total ".$mems->penz*$news;
if($paidAmount<$news*$mems->penz){
   $objheaders= new purchaseheaders();
...
$objallsavings->save();
// Loantrans for surcharge 
$objloaninfo= new loaninfo();
$objloaninfo->isInterestPay=1;

$objloantrans= new loantrans();
$objloantrans->memid=$members->memberid;
$objloantrans->surcharge=$percentage;
$objloantrans->loan=$percentage;
$objloantrans->loancredit=$percentage;
$objloantrans->date=date("Y-m-d", strtotime($today));
$objloantrans->narration=" Surcharge Auto Computation";
//$objloantrans->user=auth()->user()->name;
//$objloantrans->loanid=$memberid->loanid;
//$objloantrans->isLoan=1;
$objloantrans->branchno=$members->branchnumber;
...
// Inserting into Surcharge Recievables  
      ...
      $objaccountrans->bracid=$members->branchnumber;
      $objaccountrans->save();
