...
public function withdrawsindex(){
    ...
    $minbal=DB::select("SELECT minbal+charge as minbal,savingpdt FROM `savingsproducts` inner join savingdefinations on savingsproducts.accountcode=savingdefinations.savingac where savingsproducts.branchno=$branch");
...
$rs=savingdefinations::where('savingpdt','=','savingpdt1')->where('isActive','=',1)->where('branchno','=',$branch)->get();
    foreach($rs as $result){
        $isCharge=savingsproducts::where('accountcode','=',$result->savingac)->where('isActive','=',1)->where('branchno','=',$branch)->get();
    ...
    $savingrs=savingsproducts::where('accountcode','=',$result->savingac)->where('charge','>',0)->where('branchno','=',$branch)->get();
...
public function savingview($where,$alldate){
    $results=array();
    $bra=auth()->user()->branchid;
        ...
        $offset = ($page-1)*$rows;
        $rs =DB::getPdo();
        $krows = DB::select("select COUNT(*) as count from savings inner join customers on customers.id=savings.client_no inner join allsavings on allsavings.id=savings.savingsid $where  and moneyout>0 and isCharge!=1 and allsavings.branchno=$bra ");
        $results['total']=$krows[0]->count;
        $rst =  DB::getPdo()->prepare("select format(if(category='shares',moneyout,0),0) as shares, headerid, allsavings.id as id,purchase_headerid as header,DATE_FORMAT(savings.date,'%d-%m-%Y') as date,paydet,customers.name as name,customers.id as clino,savings.narration,format(moneyout,2) as savings,format(abs(savingpdt1),0) as savingpdt1,format(abs(savingpdt2),0) as savingpdt2,format(abs(savingpdt3),0) as savingpdt3,format(abs(savingpdt4),0) as savingpdt4,format(abs(savingpdt5),0) as savingpdt5  from savings inner join customers on customers.id=savings.client_no inner join allsavings on allsavings.id=savings.savingsid $where  and moneyout>0 and isCharge!=1 and allsavings.branchno=$bra and isFee!=1 limit $offset,$rows");
        $rst->execute();
    
        $viewall = $rst->fetchAll(\PDO::FETCH_OBJ);
       $results['rows']=$viewall;
        //Showing The footer and totals 
   $footer =  DB::getPdo()->prepare("select format(sum(if(isCharge=0,moneyout,0)),2) as savings from savings where branchno=$bra and date $alldate and isFee!=1");
   $footer->execute();
...
public function authsavingview($where){
    ...
    $bra=auth()->user()->branchid;
       ...
       $results['rows']=$viewall;
        //Showing The footer and totals 
   $footer =  DB::getPdo()->prepare("select format(sum(abs(savings)),0) as savings from loantrans inner join customers on loantrans.memid=customers.id inner join users on users.branchid=customers.branchnumber where isSavingDep=0  and narration!='Auto computed interest' loantrans.isActive=1   $where limit  $offset,$rows");
   $footer->execute();
   $foots=$footer->fetchAll(\PDO::FETCH_OBJ);
   $results['footer']=$foots;
   echo json_encode($results);
...
public function checkBalLn($clientno){
    ...
    return DB::select("select format(sum(surcharge),0) as surcharge, format(sum(loanpdt1),0) as loanpd1,format(sum(loanint1),0) as loanin1 from allsavings where  client_no=$clientno ");
}
