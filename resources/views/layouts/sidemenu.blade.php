...
{     echo "<li class=''>
            ...
            /*    if(auth::check()){
                if(auth()->user()->admin==1){
                    $result1 = DB::select("SELECT * FROM requirements inner join userroles on userroles.requirementid=requirements.id WHERE isActive=1 AND module_id=$row->id ");
                }else{
                    $result1 = DB::select("SELECT * FROM requirements inner join userroles on userroles.requirementid=requirements.id WHERE isActive=1 AND module_id=$row->id and admin=0 ");
                }
            }else{*/
                $result1 = DB::select("SELECT * FROM requirements inner join userroles on userroles.requirementid=requirements.id WHERE isActive=1 AND module_id=$row->id and userrole=$userrole ");  
               // echo "Yes";
           // }
